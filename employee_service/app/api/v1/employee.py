...
async def continue_user_signup(
    employee_data: EmployeeSelfSignup,
    db: SessionDep,
    rabbitmq: RabbitMQDep,
    current_user: TokenData = Depends(get_current_user_from_token),
):
    """Complete onboarding for the currently authenticated user."""
    payload = EmployeeCreate(
        user_id=current_user.user_id,
        employee_code="",  # auto-generated by service
        **employee_data.model_dump(),
    )
    employee = await EmployeeService.create_employee(db, rabbitmq, payload)
    return employee
